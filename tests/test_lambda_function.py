...
class QueryNotionDatabaseTests(unittest.TestCase):
    def test_query_notion_database_handles_pagination(self):
        ...
        with mock.patch("lambda_function.request_json", side_effect=[first_page, second_page]) as request_mock:
            results = lambda_function.query_notion_database("db-id", {"header": "x"}, mock.Mock())
...
class UpdateTotalAssetsCalloutTests(unittest.TestCase):
    def test_update_total_assets_callout_sets_text(self):
        block_response = {
            "callout": {
                "rich_text": [{"type": "text", "text": {"content": "Total"}}]
            }
        }
        with mock.patch("lambda_function.request_json", return_value=block_response), \
            mock.patch("lambda_function.request_status", return_value=True) as status_mock:
            lambda_function.update_total_assets_callout("block-id", 42.5, {"h": "v"}, mock.Mock())

        status_mock.assert_called_once()
        payload = status_mock.call_args.kwargs["payload"]
        self.assertEqual(payload["callout"]["rich_text"][1]["text"]["content"], ": $42.50")
